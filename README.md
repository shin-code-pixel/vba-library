Attribute VB_Name = "Mdl_VBL_Calendar"
Option Explicit
...
' - weekStartMonday: True=月曜始まり / False=日曜始まり
...
Public Function VBL_Cal_DrawMonth( _
    ...
    Optional ByVal weekStartMonday As Boolean = True _
) As Range
    ...
    If year < 1900 Or year > 9999 Then Err.Raise vbObjectError + 5003, "VBL_Cal_DrawMonth", "year が範囲外です。"
    If month < 1 Or month > 12 Then Err.Raise vbObjectError + 5004, "VBL_Cal_DrawMonth", "month が範囲外です。"
    ...
    startDow = VBL_Cal_OffsetFromWeekStart(Weekday(firstDate, vbSunday), weekStartMonday)
    ...
    ' 曜日行
    VBL_Cal_WriteWeekHeader ws, r0 + 1, c0, weekStartMonday
    ...
    For r = 0 To weeks - 1
        For c = 0 To 6
            With ws.Cells(r0 + 2 + r, c0 + c)
                ...
                ' 土日を薄く区別（色は最小限）
                If VBL_Cal_IsWeekend(cur, weekStartMonday) Then
                    .Font.Bold = True
                Else
                    .Font.Bold = False
                End If
...
' 使い方例（デモ）
'============================================================
Public Sub VBL_Cal_Demo()
    Dim ws As Worksheet
    Set ws = ActiveSheet

    ws.Cells.Clear
    Call VBL_Cal_DrawMonth(ws, Year(Date), Month(Date), ws.Range("B2"), True)
End Sub
